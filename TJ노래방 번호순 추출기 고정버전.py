...
def tjmediasongnumberfindingaddressmaker(x):
    ...
    print(f"Found {len(sets)//3} songs matching the query parameter: {x}")
    return sets

# Function to periodically write data to an Excel file
def write_to_excel(data, file_name):
    df = pd.DataFrame.from_records(data, columns=['Song Number', 'Song Name', 'Singer Name'])
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame.from_records([combined_data], columns=['Song Number', 'Song Name', 'Singer Name'])
